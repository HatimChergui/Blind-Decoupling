...
# ML algorithms
    ...
    svm = SVC(kernel='rbf', C=1, cache_size=1000, random_state=0, decision_function_shape='ovo')
    ...
    acu_score_avg.append((acu_score>=0.9).mean())
    sc = np.array(acu_score_avg, np.float)
    decoupling_success_rate.append(np.mean(sc))
...
tck = interpolate.splrep(n_split, decoupling_success_rate, s=0)
xnew = np.arange(50,N_training_max+1,50)
ynew = interpolate.splev(xnew, tck, der=0)

threshold_decision_success = 0.73*np.ones((60))

plt.plot(xnew, ynew,'b', label="SVM with RBF")
